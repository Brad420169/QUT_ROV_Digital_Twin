...
from camera_thermal import VisionPermission
...
class GamepadTeleop(Node):
    def __init__(self):
        ...
        self.vision_permission = VisionPermission()
        ...
        self.follow_enable_pub = self.create_publisher(
            Bool,
            '/qut_rov/tennis_ball_enabled' if self.rov_mode == 'real' else FISH_FOLLOW_ENABLE_TOPIC,
            10,
        )
        self.vision_health_sub = self.create_subscription(
            Bool, '/qut_rov/vision_control_allowed',
            lambda msg: self.vision_permission.update(msg.data), 1)
        ...
        self.fish_cmd_sub = self.create_subscription(
            Twist,
            '/qut_rov/tennis_ball_cmd' if self.rov_mode == 'real' else FISH_FOLLOW_CMD_TOPIC,
    ...
    def fish_command_callback(self, msg: Twist):
        if self.rov_mode == 'real' and not self.vision_permission.valid():
            return
    ...
    def toggle_fish_follow(self):
        if self.rov_mode == "real":
            self._set_fish_follow(not self.fish_follow_mode)
            return
    ...
    def _set_fish_follow(self, enabled: bool):
        if enabled and self.rov_mode == 'real' and not self.vision_permission.valid():
            self.get_logger().warning('Vision control denied: camera hot, slow, or health unavailable.')
            enabled = False
        if enabled:
    ...
    def publish_command(self):
        if self.rov_mode == 'real' and self.fish_follow_mode and not self.vision_permission.valid():
            self._set_fish_follow(False)
        ...
        for source in required:
            if not self.inputs[source].fresh():
                ...
                break
        if self.rov_mode == 'real':
            # Tracker requires a live controller heartbeat, including while disabled.
            self.follow_enable_pub.publish(Bool(data=bool(
                self.fish_follow_mode and not self._manual_rearm and not self._stopped)))
        msg = Twist()
